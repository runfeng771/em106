...
def _geo_lookup(ip: str) -> dict:
    ...
    if IP2LOCATION_KEY:
        try:
            ...
            # 注意：不要把 key 暴露到前端或仓库代码里，务必用环境变量注入
            r = requests.get(
...
<body>
  ...
  <div class="card">
    ...
    <div class="row" style="margin-top:10px; flex-wrap: wrap; gap: 10px;">
      ...
      <span class="chip" id="visitorOS">系统: --</span>
      <span class="chip" id="visitorDevice">设备: --</span>
      <span class="chip" id="visitorUA" title="User-Agent">UA: --</span>
    </div>
...
async function refreshOnline(){
  try{
    ...
    const osEl  = document.getElementById('visitorOS');
    const devEl = document.getElementById('visitorDevice');
    const uaEl  = document.getElementById('visitorUA');
    ...
    if(brEl) brEl.textContent = '浏览器: ' + (j.browser || '--');
    if(osEl) osEl.textContent = '系统: ' + (j.os || '--');
    if(devEl) devEl.textContent = '设备: ' + (j.device || '--');

    const ua = (j.user_agent || '').trim();
    if(uaEl){
      const shortUA = ua ? (ua.length > 60 ? (ua.slice(0, 60) + '…') : ua) : '--';
      uaEl.textContent = 'UA: ' + shortUA;
      uaEl.title = ua || 'User-Agent';
    }
  }catch(e){
    // ignore
  }
